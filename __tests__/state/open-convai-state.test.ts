...
import { HCS10BaseClient, IConnectionsManager, Connection, Logger, AIAgentProfile } from '@hashgraphonline/standards-sdk';
import { ActiveConnection, RegisteredAgent, AgentPersistenceOptions } from '../../src/state/state-types';
...
describe('OpenConvaiState', () => {
  ...
  beforeEach(() => {
    ...
    mockBaseClient = {
      ...
      requestAccount: jest.fn<() => Promise<any>>().mockResolvedValue({ balance: { balance: 0 } } as any),
  ...
  describe('constructor and initialization', () => {
    it('should initialize with default envFilePath and prefix', () => {
      ...
      expect((state as any).logger).toBeInstanceOf(Logger);
    });

    it('should initialize with provided options', () => {
      const opts = { defaultEnvFilePath: '.env.test', defaultPrefix: 'TEST', baseClient: mockBaseClient };
    ...
    it('should initialize ConnectionsManager when initializeConnectionsManager is called', () => {
      ...
      expect(cm.constructor.name).toBe('ConnectionsManager');
    });
  ...
  const ensureConnectionsManager = () => {
    if (!state.getConnectionsManager()) {
      connectionsManagerInstance = state.initializeConnectionsManager(mockBaseClient);
    } else {
  ...
  describe('setCurrentAgent', () => {
    it('should set the current agent and clear timestamps', () => {
      ...
      const agent: RegisteredAgent = { name: 'TestAgent', accountId: '0.0.1', inboundTopicId: '0.0.2', outboundTopicId: '0.0.3' };
  ...
  describe('addActiveConnection', () => {
    it('should throw if ConnectionsManager is not initialized', () => {
        ...
        const conn: ActiveConnection = { targetAccountId: '0.0.10', connectionTopicId: '0.0.11', targetAgentName:'', targetInboundTopicId:'' };
        expect(() => state.addActiveConnection(conn)).toThrow('ConnectionsManager not initialized');
    });
    ...
    it('should call connectionsManager.updateOrAddConnection and initialize timestamp', () => {
      ...
      const calledWith = (cm.updateOrAddConnection as jest.Mock).mock.calls[0][0] as Connection;
  ...
  describe('listConnections', () => {
    ...
    it('should call connectionsManager.getAllConnections and convert results', () => {
      ...
      const sdkConn: Connection = {
        connectionTopicId: '0.0.1', targetAccountId: '0.0.2', status: 'established',
        created: new Date(), isPending: false, needsConfirmation: false, processed: true
      };
  ...
  describe('getConnectionByIdentifier', () => {
    ...
    const sdkConn1: Connection = { connectionTopicId: 'topic1', targetAccountId: 'acc1', status: 'established', created: new Date(), isPending:false, needsConfirmation:false, processed:true, targetAgentName:'Agent1', targetInboundTopicId:'inTopic1' };
    const sdkConn2: Connection = { connectionTopicId: 'topic2', targetAccountId: 'acc2', status: 'pending', created: new Date(), isPending:true, needsConfirmation:false, processed:true, targetAgentName:'Agent2', targetInboundTopicId:'inTopic2' };
    ...
    beforeEach(() => {
      ...
      jest.spyOn(cm, 'getConnectionByTopicId').mockImplementation(id => {
        if (id === 'topic1') { return sdkConn1; }
        if (id === 'topic2') { return sdkConn2; }
        return undefined;
      });
      jest.spyOn(cm, 'getConnectionByAccountId').mockImplementation(id => {
        if (id === 'acc1') { return sdkConn1; }
        if (id === 'acc2') { return sdkConn2; }
        return undefined;
      });
    });
    ...
    it('should find by account ID if not found by topic ID', () => {
      (cm.getConnectionByTopicId as jest.Mock).mockImplementation(id => {
        if (id === 'acc2') { return undefined; } // Simulate not found by topic for 'acc2'
        return undefined;
      });
  ...
  describe('timestamp management', () => {
    ...
    it('should initialize timestamp on first addActiveConnection for a topic', () => {
      state.addActiveConnection({ connectionTopicId: topicId, targetAccountId: 'any', status: 'established', targetAgentName:'', targetInboundTopicId:'' });
  ...
  describe('status conversion (internal helpers)', () => {
    it('convertToActiveConnection should map SDK Connection to ActiveConnection correctly', () => {
        const sdkConn: Connection = {
            connectionTopicId: 't1', targetAccountId: 'a1', targetAgentName: 'Agent X',
            targetInboundTopicId: 'in1', status: 'established', created: new Date(),
            lastActivity: new Date(), isPending: false, needsConfirmation: false,
            profileInfo: {} as AIAgentProfile,
            connectionRequestId: 123, processed: true
        };
    ...
    it('should correctly map SDK statuses to state statuses via convertToStateStatus', () => {
        ...
        expect((state as any).convertToStateStatus('established')).toBe('established');
        expect((state as any).convertToStateStatus('needs_confirmation')).toBe('needs confirmation');
        expect((state as any).convertToStateStatus('closed')).toBe('established');
        expect((state as any).convertToStateStatus('unknown_status')).toBe('unknown');
    });
  });
  ...
  describe('persistAgentData', () => {
    ...
    const options: AgentPersistenceOptions = { type: 'env-file', envFilePath: '.env.test.persist', prefix: 'TEST_AGENT' };
    let mockUpdateEnvFile: jest.Mock;

    beforeEach(() => {
      const mockedStateTools = jest.requireMock('../../src/utils/state-tools') as {
    ...
    it('should throw error for unsupported persistence type', async () => {
      const invalidOptions: AgentPersistenceOptions = { type: 'unsupported' as any };
      await expect(state.persistAgentData(agent, invalidOptions))
        .rejects.toThrow('Unsupported persistence type: unsupported. Only \'env-file\' is supported.');
    });

    it('should throw error if agent data is incomplete', async () => {
      const incompleteAgent: RegisteredAgent = { ...agent, accountId: undefined as any };
      await expect(state.persistAgentData(incompleteAgent, options))
    ...
    it('should use default prefix if not provided in options', async () => {
        const optsNoPrefix: AgentPersistenceOptions = { type: 'env-file', envFilePath: '.env.test.persist'};
